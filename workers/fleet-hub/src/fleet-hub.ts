...
//
// Security boundaries:
//   * /publish is HMAC-gated. We require
//     X-Signature == hex-sha256(body || '\n' || ts) with X-Timestamp
//     (unix-seconds) carried alongside; the request is accepted only when
//     |now - ts| <= 30s. Constant-time comparison guards timing oracles.
...
export class FleetHub extends DurableObject<Env> {
  ...
  // via verifyPublishHMAC: the signature is HMAC over `body || '\n' || ts`
  // with `X-Timestamp` carrying unix-seconds and a ±30s window enforced
  // against `now`. Outside that window — or with no X-Timestamp — the
  // request is rejected, so a captured signed body cannot be replayed
  // past the window. On success we broadcast the event to every
  // currently-connected WS.
...
// /internal/publish. The envelope is `HMAC-SHA256(body || '\n' || ts,
// secret)` with `X-Timestamp` carrying the unix-seconds string. Accepted
// only when `|now - ts| <= 30s`. Outside that window — or with no
// X-Timestamp — the request is rejected. A captured signed body is
// therefore unusable past the window.
//
...
export async function verifyPublishHMAC(
  ...
  // Build `body + '\n' + ts` as a single byte buffer. Encoding ts as UTF-8
  // keeps the byte layout identical to a Go-side append(body, '\n', ts).
  ...
  return { ok: timingSafeEqualHex(signature, expected) }
}
