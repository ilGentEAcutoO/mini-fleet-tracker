...
- **CSP can be tight.** The deployed policy is `default-src 'self'; script-src 'self'; connect-src 'self' wss://fleet-tracker.jairukchan.com https://tiles.openfreemap.org; worker-src 'self' blob:; img-src 'self' data: https:; style-src 'self' 'unsafe-inline'; frame-ancestors 'none'; base-uri 'self';`. Map tiles come from OpenFreeMap's vector-tile CDN (no API key, no rate limit) which keeps the policy short — no third-party SDK loaders in `script-src`. MapLibre uses a web worker to decode vector tiles, hence `worker-src 'self' blob:`. The gateway Worker injects this header on every HTML response so the policy lives next to routing, not buried in a Nuxt plugin.
